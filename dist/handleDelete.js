...
export const getHandleDelete = ({ purge, storage, stream })=>{
    return async ({ collection, doc, filename, req })=>{
        try {
            ...
            if (stream && video) {
                await ky.delete(`https://video.bunnycdn.com/library/${stream.libraryId}/videos/${video.videoId}`, {
                    headers: {
